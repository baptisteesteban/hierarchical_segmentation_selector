...
class LCA:
	...
	def _block_signature(depths: list[int], start: int, end: int) -> int:
		...
		for idx in range(start + 1, end):
			...
			if delta not in (1, -1):
				raise ValueError("Euler depth array is not +/-1 between consecutive entries")
	...
	def _build_micro_table(block_kind: _BlockKind) -> list[int]:
		...
		for idx in range(1, length):
			...
			relative_depths[idx] = relative_depths[idx - 1] + (1 if step_is_plus else -1)
	...
	def _build_rmq_index(self) -> None:
		...
		self._block_starts = [block * self._block_size for block in range(self._num_blocks)]
		self._block_lengths = [
	...
	def _rmq_query(self, left: int, right: int) -> int:
		...
		if left_block == right_block:
			return self._query_block(left_block, left % self._block_size, right % self._block_size)
